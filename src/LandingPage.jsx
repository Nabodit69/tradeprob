...
export default function LandingPage({ onSignIn }) {
  ...
  const handleDownloadHTML = () => {
    alert(
      "HTML export is not available in this preview. Integrate the exportHtml utility from your project to enable this feature."
    );
  };

  const calculateConsecutiveStats = (rate) => {
    const lossProb = (100 - rate) / 100;
    const winProb = rate / 100;
    const expectedLongestLossStreak = Math.max(
      1,
      Math.round(Math.log(simTrades) / -Math.log(lossProb))
    );
    const expectedLongestWinStreak = Math.max(
      1,
      Math.round(Math.log(simTrades) / -Math.log(winProb))
    );
    return {
      lossStreak: expectedLongestLossStreak,
      winStreak: expectedLongestWinStreak,
      winProbSeries: (Math.pow(winProb, 5) * 100).toFixed(1),
      lossProbSeries: (Math.pow(lossProb, 5) * 100).toFixed(1),
    };
  };
  ...
  return (
    <div
      ...
      className="min-h-screen bg-slate-50 text-slate-900 selection:bg-blue-600 selection:text-white font-sans antialiased overflow-x-hidden flex flex-col justify-between"
    >
      {/* Navigation Bar */}
      <nav
        id="landing-nav"
        className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-slate-200/80 px-6 py-4"
      >
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          ...
          >
            ...
            <div>
              ...
              <span className="block text-[9px] text-blue-600 font-bold uppercase tracking-wider -mt-1">
          ...
          <div className="hidden lg:flex items-center gap-1 bg-slate-100/80 p-1 rounded-2xl border border-slate-200/50">
            {["home", "plans", "blog", "affiliate"].map((tab) => (
              <button
                key={tab}
                id={`nav-tab-${tab}`}
                onClick={() => {
          ...
          <div className="hidden lg:flex items-center gap-4">
            <button
              ...
              className="text-xs font-bold uppercase tracking-wider text-blue-600 border border-blue-200 hover:border-blue-300 hover:bg-blue-50 px-4 py-2.5 rounded-xl transition-all duration-200 flex items-center gap-2 shadow-sm"
            >
              <Download size={14} />
              ...
              <div className="flex flex-col gap-2 pt-2 pb-4">
                ...
                <div className="flex flex-col gap-3 p-2">
                  <button
                    ...
                    className="w-full text-center border-2 border-dashed border-blue-300 py-3 rounded-xl text-xs font-bold uppercase tracking-wider text-blue-600 bg-blue-50/50 hover:bg-blue-50 transition-colors flex items-center justify-center gap-2"
                  >
                    <Download size={14} />
                  ...
                  <div className="flex gap-4">
                    ...
                    <button
                      ...
                      className="flex-1 bg-blue-600 text-white py-3 rounded-xl text-xs font-bold uppercase tracking-wider hover:bg-blue-700 transition-colors shadow-md shadow-blue-500/10 flex items-center justify-center gap-1.5"
                    >
              ...
              >
                <div className="lg:col-span-7 space-y-6 text-center lg:text-left">
                  ...
                  <p className="text-slate-600 text-base md:text-lg max-w-2xl mx-auto lg:mx-0 leading-relaxed">
                    Ditch emotional triggers. Track multiple trading strategy
                    catalogs with a dynamic real-time probability matrix that
                    isolates edge levels, streak models, and risk distributions.
                  </p>
              ...
              <section id="features-section" className="space-y-12">
                <div className="text-center max-w-3xl mx-auto space-y-4">
                  <p className="text-xs font-black uppercase tracking-widest text-blue-600">
                    Built for Serious Disciplined Traders
                  </p>
                  <h2 className="text-3xl md:text-4xl font-black text-slate-900 tracking-tight">
                    Gain a strict mathematical layout for your portfolio edge
                  </h2>
                  <p className="text-slate-500 leading-relaxed">
                    Stop trading randomly. Map setups, keep visual catalogs, and
                    view probabilities live. Our algorithms process distribution
                    curves derived from your actual database records.
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                  <div className="bg-white border border-slate-200 p-8 rounded-3xl space-y-4 hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
                    ...
                    <h3 className="text-lg font-bold text-slate-900">
                      Setup Probability Models
                    </h3>
                    <p className="text-slate-500 text-sm leading-relaxed">
                      Automatically calculate performance metrics across
                      variables like Breakouts, Pullbacks, Range trades and mean
                      reversion setups.
                    </p>
                  </div>
                  <div className="bg-white border border-slate-200 p-8 rounded-3xl space-y-4 hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
                    ...
                    <p className="text-slate-500 text-sm leading-relaxed">
                      Predict and prepare for drawdowns. Determine whether a
                      streak of consecutive losses is normal or a structural
                      breakdown in your current setup system.
                    </p>
                  </div>
                  <div className="bg-white border border-slate-200 p-8 rounded-3xl space-y-4 hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
                    <div className="w-12 h-12 rounded-2xl bg-blue-50 text-blue-600 flex items-center justify-center shadow-inner">
                    ...
                    <h3 className="text-lg font-bold text-slate-900">
                      Instant Clean CSV Exports
                    </h3>
                    <p className="text-slate-500 text-sm leading-relaxed">
                      Your trades are your assets. Download fully indexed CSV
                      data with screenshots, stop loss values, takes profits,
                      and emotional notes seamlessly on hover.
                    </p>
                  </div>
                </div>
              </section>
              ...
              <section className="bg-slate-900 text-white rounded-[3rem] p-8 md:p-16 relative overflow-hidden shadow-2xl">
                ...
                <div className="relative max-w-3xl space-y-6">
                  ...
                  <p className="text-slate-400 text-base md:text-lg leading-relaxed">
                    With zero complex configurations needed. Connect with your
                    Google Account, define your customized catalog parameters,
                    and let our mathematics isolate consistent pips generators.
                  </p>
                  <div className="pt-4">
                    <button
                      onClick={onSignIn}
            ...
            >
              <div className="text-center max-w-xl mx-auto space-y-4">
                ...
                <h1 className="text-4xl font-black text-slate-900 tracking-tight">
                  Flexible plans for any trading scale
                </h1>
                <p className="text-slate-500 max-w-md mx-auto leading-relaxed">
                  Choose the subscription duration that fits your strategy
                  testing goals. Cancel or extend anytime.
                </p>
              </div>
              ...
              >
                ...
                <div className="bg-white border border-slate-200 rounded-[2.5rem] p-8 flex flex-col justify-between hover:shadow-xl transition-all duration-300 relative">
                  <div className="space-y-6">
                    <div className="space-y-3">
                      <span className="text-[10px] font-black uppercase bg-slate-100 text-slate-600 px-3 py-1 rounded-full w-max block">
                        Explorer tier
                      </span>
                      ...
                      <p className="text-slate-400 text-xs">
                        Perfect for quick strategy testing & validation.
                      </p>
                    </div>
                    ...
                    <ul className="space-y-4 border-t border-slate-100 pt-6 text-xs text-slate-600 font-semibold">
                      {[
                        "All core strategies calculators",
                        "Real-time streak distribution models",
                        "Interactive journal & trade logs",
                        "Export CSV & TradingView links",
                      ].map((f) => (
                ...
                <div className="bg-white border-2 border-blue-600 rounded-[2.5rem] p-8 flex flex-col justify-between hover:shadow-2xl hover:scale-[1.02] transition-all duration-300 relative shadow-xl shadow-blue-500/5">
                  ...
                  <div className="space-y-6">
                    <div className="space-y-3">
                      <span className="text-[10px] font-black uppercase bg-blue-50 text-blue-600 px-3 py-1 rounded-full w-max block">
                        Ultimate commitment
                      </span>
                      <h3 className="text-2xl font-black text-slate-900">
                        6 Months
                      ...
                      <p className="text-slate-400 text-xs">
                        Ideal choice for systematic traders aiming for
                        consistent edge.
                      </p>
                    </div>
                    <div className="border-t border-slate-100 pt-6">
                    ...
                    <ul className="space-y-4 border-t border-slate-100 pt-6 text-xs text-slate-600 font-semibold">
                      {[
                        ...
                        "Priority server syncing speeds",
                        "Multiple concurrent strategy groups",
                        "Advanced probability distributions specs",
                      ].map((f) => (
                        <li key={f} className="flex items-center gap-2">
                          <Check size={16} className="text-blue-600" />
                          <span>{f}</span>
                ...
                <div className="bg-white border border-slate-200 rounded-[2.5rem] p-8 flex flex-col justify-between hover:shadow-xl transition-all duration-300 relative">
                  <div className="space-y-6">
                    <div className="space-y-3">
                      <span className="text-[10px] font-black uppercase bg-emerald-50 text-emerald-600 px-3 py-1 rounded-full w-max block">
                        Best Value (Save 50%)
                      </span>
                      <h3 className="text-2xl font-black text-slate-900">
                        12 Months
                      ...
                      <p className="text-slate-400 text-xs">
                        For professional traders seeking a long-term compound
                        strategy.
                      </p>
                    </div>
                    <div className="border-t border-slate-100 pt-6">
                    ...
                    <ul className="space-y-4 border-t border-slate-100 pt-6 text-xs text-slate-600 font-semibold">
                      {[
                        ...
                        "Infinite historical strategy backtests",
                        "Premium dedicated developer logs",
                        "Lifetime feature updates access",
                      ].map((f) => (
                        <li key={f} className="flex items-center gap-2">
                          <Check size={16} className="text-blue-600" />
                          <span>{f}</span>
              ...
              <div className="max-w-3xl mx-auto border-t border-slate-200/80 pt-16 space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-sm">
                  {[
                    {
                      q: "How does the 6-month billing compute?",
                      a: "It bills as a single semi-annual charge of ₹894 (₹149 × 6). This saves you 25% over the monthly explorer pricing rate.",
                    },
                    {
                      q: "Can I transfer strategies between plans?",
                      a: "Yes, your entire data is backed up securely under your Google account credentials, regardless of plan changes.",
                    },
                    {
                      q: "Is stop-loss and take-profit math automated?",
                      a: "Yes. The system parses your target parameters and alerts if actual performance diverges heavily from expectancy.",
                    },
                    {
                      q: "What is the money-back policy?",
                      a: "We offer a full, no-questions-asked refund within 14 days of activation if you feel the statistics are not aiding your edge.",
                    },
                  ].map(({ q, a }) => (
                    <div key={q} className="space-y-2">
                      <p className="font-bold text-slate-900">{q}</p>
                      <p className="text-slate-500 font-medium">{a}</p>
                    </div>
                  ))}
                </div>
              </div>
          ...
          {activeTab === "blog" && (
            ...
            >
              <div className="text-center max-w-xl mx-auto space-y-4">
                ...
                <p className="text-slate-500 leading-relaxed">
                  Systematic trading advice, stop loss distribution curves, and
                  the ultimate math guidebooks to beating emotional trade loops.
                </p>
              </div>
                ...
                {[
                  {
                    cat: "Psychology",
                    time: "5 Min Read",
                    title: "Beating the Drawdown Loop: Why Consecutives Occur",
                    desc: "Understanding sample series error margins and why even 70% win-rate formulas experience consecutive losses.",
                  ...
                  },
                  {
                    cat: "Mathematics",
                    time: "8 Min Read",
                    title: "Risk Reward Ratio (R:R) vs. Realized Percent Expectancy",
                    desc: "How setting strict stop losses parameters and target pips saves your account from total variance decay.",
                  ...
                  },
                  {
                    cat: "Log Keeping",
                    time: "4 Min Read",
                    title: "Why Taking Screenshots of TradingView is Non-Negotiable",
                    desc: "Visual memory mapping strategies and emotional tracking that help you review previous trading models with clarity.",
                    author: "Dr. Sarah Vance",
                    date: "Aug 24, 2026",
                  },
                ].map((post) => (
                  <div
                    key={post.title}
                    className="bg-white border border-slate-200 rounded-[2rem] p-6 space-y-4 flex flex-col justify-between grayscale opacity-80 relative overflow-hidden"
                  >
                    <div className="absolute top-4 right-4 bg-blue-600 text-white text-[9px] font-black uppercase px-2.5 py-1 rounded-lg tracking-wider z-10">
                      COMING SOON
                    </div>
                    <div className="space-y-3">
              ...
              <div className="max-w-xl mx-auto bg-white border border-slate-200 p-8 rounded-3xl text-center space-y-6 shadow-md shadow-slate-100/50">
                ...
                <div className="space-y-2">
                  <h3 className="font-bold text-slate-900 text-lg">
                    Subscribe to get notified major guidebooks publish
                  </h3>
                  <p className="text-slate-500 text-xs">
                    No spam. Only deep mathematical breakdowns and strategy
                    spreadsheets.
                  </p>
                </div>
                {emailSubscribed ? (
          ...
          {activeTab === "affiliate" && (
            ...
            >
              <div className="text-center max-w-xl mx-auto space-y-4">
                ...
                <h1 className="text-4xl font-black text-slate-900 tracking-tight">
                  TradeProb Affiliate Program
                </h1>
                <p className="text-slate-500 leading-relaxed">
                  Earn high recurring commissions by sharing TradeProb tools and
                  dashboards with your trading audience, discord networks, and
                  newsletter channels.
                </p>
              </div>
              ...
              >
                {[
                  ...
                  },
                  {
                    ...
                    val: "Fast Monthly",
                    label: "Payouts via USDT/Bank",
                  },
                ].map(({ icon, bg, val, label }) => (
                  <div
                    key={label}
                    className="bg-white border border-slate-200 p-6 rounded-2xl text-center space-y-2"
                  >
                    <div
                      className={`w-10 h-10 rounded-xl ${bg} flex items-center justify-center mx-auto`}
                    >
                      {icon}
                    </div>
                    <h4 className="text-2xl font-black text-slate-900">{val}</h4>
              ...
              <div
                ...
                className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center bg-white border border-slate-200 p-8 rounded-[2.5rem]"
              >
                <div className="lg:col-span-7 space-y-6">
                  <span className="text-[10px] font-black uppercase text-amber-600 bg-amber-50 px-3 py-1 rounded-full w-max block">
                    Program Status: Coming Soon
                  </span>
                  <h3 className="text-2xl font-black text-slate-900">
                    Maximize passive profit curves by helping traders refine
                    their expectancy levels.
                  </h3>
                  <p className="text-slate-500 text-sm leading-relaxed">
                    When the affiliate engine launches soon, you will receive a
                    customizable link tracking cookie records instantly. We
                    provide high-converting marketing content, direct backtest
                    graphs, and landing page scripts to embed directly.
                  </p>
                  <div className="space-y-3 font-semibold text-xs text-slate-700">
                    {[
                      "Dedicated partnership review panel",
                      "Elite resource bundlepacks with strategy guides",
                      "Personal VIP manager for referrals above 50 accounts",
                    ].map((item) => (
                ...
                <div className="lg:col-span-5 bg-slate-50 border border-slate-200 p-6 rounded-3xl space-y-6">
                  <div className="space-y-2 text-center lg:text-left">
                    ...
                    <p className="text-slate-500 text-xs">
                      Get priority approval and 35% starting rates instead of
                      30%.
                    </p>
                  </div>
                  {affiliateJoined ? (
      ...
      <footer
        id="landing-footer"
        className="bg-white border-t border-slate-200 px-6 py-8 mt-12"
      >
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4 text-center">
          ...
          <p className="text-slate-400 text-xs max-w-md">
            Trading derivatives carries substantial risk. All values derived
            from simulations represent statistical likelihoods and are for
